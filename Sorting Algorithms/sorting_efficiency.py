...
def merge_sort(seq):
	tmp = [0 for i in seq]
	merge_sort_rec(seq, 0, len(seq), tmp)
...
def merge(seq, min_v, mid_v, max_v, tmp):
	...
	curr_r = mid_v
	curr_t = min_v
	while curr_l < mid_v and curr_r < max_v:
...
def merge_slow(seq, min_v, mid_v, max_v, tmp):
	curr_l = min_v
	curr_r = mid_v
	curr_t = min_v
	tmp = [ i for i in seq ]
...
def radix_sort(seq):
	...
	for i in range(max_length):
		...
		for j in seq:
			radix = (j / 10**i) % 10
			if buckets.has_key(radix):
				buckets[radix].append(j)
			else:
				buckets[radix] = [j]
		seq=[]
		for j in range(10):
			if buckets.has_key(j):
				seq += buckets[j]
	return seq		
